...
[ApiController]
[Route("organisations/{organisationId:guid}/roles")]
[Authorize]
public class OrganisationRoleController(IOrganisationRoleService service) : ControllerBase
{
    [HttpGet]
    ...
    [HttpPost]
    public async Task<IActionResult> Create(Guid organisationId, [FromBody] CreateOrganisationRoleDTO dto)
        => Ok(await service.Create(organisationId, dto));

    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid organisationId, Guid id, [FromBody] UpdateOrganisationRoleDTO dto)
    {
